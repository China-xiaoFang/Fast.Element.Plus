...
13 个插件方法的独立调用示例、输入和返回值表见[方法 API](./methods)；本页继续负责完整选项字段和行为分支。
...
组合使用时必须保持 `subresourceIntegrity` → `bundleBudget` → `compression`，配置解析阶段会主动诊断逆序。部署与信任边界见[风险指南](./risks)。
...
公开类型按插件分组如下；每个 Options 的字段、默认值与使用方式已在对应插件章节的表格和示例中说明。

- `buildInfo`：`BuildInformation`、`BuildInfoContext`、`BuildInfoPluginOptions`。
- `bundleBudget`：`BundleSizeMode`、`BundleOutputType`、`BundleBudgetFilter`、`BundleBudgetRule`、`BundleBudgetPluginOptions`、`MeasurableBundleOutput`、`BundleBudgetFileMeasurement`、`BundleBudgetEvaluation`。
- `cdnImport`：`CdnTagAttributes`、`CdnModule`、`CdnModuleResolverContext`、`CdnModuleResolver`、`ResolvedCdnModule`、`CdnImportPluginOptions`。
- `componentRegistry`：`ComponentNameContext`、`ScannedComponent`、`ComponentRegistryPluginOptions`。
- `compression`：`CompressionAlgorithm`、`CompressionPluginOptions`。
- `devRestart`：`DevRestartEvent`、`DevRestartContext`、`DevRestartPluginOptions`、`ResolvedRestartPath`。
- `envGuard`：`EnvRule`、`EnvSchema`、`EnvValidationIssue`、`EnvGuardPluginOptions`。
- `htmlTemplate`：`HtmlTemplateValue`、`HtmlTemplateData`、`HtmlTemplateContext`、`HtmlTemplatePluginOptions`。
- `routerMeta`：`RouterMetaContext`、`RouterMetaPluginOptions`、`RouterMetaMap`。
- `staticCopy`：`StaticCopyTarget`、`StaticCopyPluginOptions`。
- `subresourceIntegrity`：`SubresourceIntegrityAlgorithm`、`SubresourceIntegrityFilter`、`IntegrityCrossorigin`、`SubresourceIntegrityPluginOptions`、`IntegrityInjectionOptions`、`IntegrityInjectionResult`。
- `svgIcons`：`SvgAttributeValue`、`ParsedSvg`、`SvgIconNameContext`、`ScannedSvgIcon`、`SvgIconsPluginOptions`。
- `virtualModules`：`VirtualModuleContext`、`VirtualModuleSource`、`VirtualModuleMap`、`VirtualModulesPluginOptions`。

这些类型均具备 TSDoc，可在编辑器中查看回调上下文、结果字段、错误行为和安全约束。内部 `shared/` Helper 未从根入口导出，不属于公开 API。
