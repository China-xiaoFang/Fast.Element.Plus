...
单图上传，固定使用单文件模式，支持固定宽高、替换、删除、预览和 `string | null` 模型。默认关闭拖拽上传；需要拖拽时可显式设置 `drag`。在手动上传模式下，选择文件时会立即校验文件类型和大小。`width`、`height` 控制组件实际尺寸，上传区域会在总高度内为提示文字保留空间；边框颜色沿用 Element Plus 主题。
