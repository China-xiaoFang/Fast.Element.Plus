...
在 ElDialog 上统一拖动、全屏、刷新、Footer、异步打开/关闭和业务 Loading。调用 Expose `open(function?)`、`close(function?)` 执行业务钩子并返回 `Promise`；`v-model` 只接收内部状态回写，外部赋值不会打开或关闭 Dialog。`open`、`close` 是 Fast 业务流程完成事件，`opened`、`closed` 仍保留 Element Plus 动画生命周期。
