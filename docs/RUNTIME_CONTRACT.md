...
- Framework boundary: Vue, Element Plus, Element Plus Icons, and Fast.Element.Plus.Icons remain application-provided peers. Both icon packages are required and external to the build. Component-only utilities are bundled and are not public package dependencies.
...
- `FastElementPlus` is the component namespace; internal component implementation modules are not public paths.
...
- 框架边界：Vue、Element Plus、Element Plus Icons 和 Fast.Element.Plus.Icons 均由应用作为强制 Peer 提供；两套图标包保持在构建产物之外。组件内部工具已打包，且不构成公共依赖。
...
默认导出和具名 `install` 用于完整安装；组件、指令、Hook、常量、Decimal、实例类型、Props、Emits、Slots、公开方法和辅助类型统一从根入口具名导出。`FastElementPlus` 是组件命名空间，内部实现模块不是公共路径。
